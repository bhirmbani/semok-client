      ...
      if (items === null) {
        ...
        .then((categoryRef) => {
          if (categoryRef === null) {
            ...
            .then((category) => {
              ...
              .then(() => {
                res.json({ msg: `sukses menambahkan kategori ${req.body.name} pada item dengan id ${req.body.itemId}`, ok: true });
              });
            });
          } else {
            items.update({
            ...
            .then((itemWithCategory) => {
              res.json({ itemWithCategory, categoryRef, msg: `kategori ${req.body.name} sudah ada. Terletak pada id ${categoryRef.id}`, msg2: `sukses menambahkan kategori ${req.body.name} pada item ${itemWithCategory.name}`, ok: true });
            });
          }
        });
      } else {
        ...
        .then((categories) => {
          ...
          if (filteredCategory.length < 1) {
            ...
            .then((updatedItem) => {
              res.json({ updatedItem, filteredCategory, msg: `sukses mengubah kategori untuk item ${items.name} menjadi kategori dengan id ${categoryId}`, ok: true });
            });
          }
        });
      }
...
methods.gets = (req, res, next) => {
  if (!req.headers.token) {
    ...
    .then((categories) => {
      res.json({ categories, msg: 'berhasil dapatkan data semua kategori' });
    });
  }
};
  ...
  if (!req.headers.token) {
    ...
    .then((category) => {
      if (category === null) {
        ...
        res.json({ msg: 'sudah ada kategori dengan nama ini', ok: false });
      }
    });
  }
};
